...
**Val Core Phase 1, 11 September 2026.** The boundary types themselves —
`ProviderResult`, `ProviderAdapter`, and the streaming contract (`TextDelta`,
`ProviderEvent`, `StreamingProviderAdapter`, `DeltaSink`) — live in
`val_domain.provider`, so that the core depends on the boundary and the
providers depend on the core's terms. They are re-exported here unchanged for
every existing import; the error normalizer below stays here, since it is
the one piece of the boundary that knows what SDK exceptions look like.
"""
