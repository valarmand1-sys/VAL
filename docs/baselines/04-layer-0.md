...
>
> *Term quality:* assessed on the store, read-only (`VAL_Turn_Path_Inventory.md` §6): document frequency per lexeme is computable in the recall scope from the existing GIN index at ~10 ms per term; no governing text determines a threshold, so **no floor is deployed** and the recommendation is returned for ruling. The AND-to-OR rewrite is not reopened.
>
