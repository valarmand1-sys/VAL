...
>
> **The strip is its own capability floor (same ruling; `01-architecture.md` §5.2):** `CapabilityProfile.STRIP`, required by the strip task and held only by configurations that have demonstrated the strip contract on the frozen conformance suite — zero preference leakage into an enforced blind payload, zero unsupported attributed-prior leakage where removal is required, zero neutral-decision-content removal, zero genuinely inseparable cases reaching enforced ordering, exact expected residue on every separable case (allowing only recorded punctuation alternatives), and no false contamination on clearly separable cases. Claude Haiku 4.5 holds it only until a demonstrated successor is designated; its record on the suite (35 of 112 conformant, 27 false contaminations, 50 blocking, removal of neutral instructions such as "Decide and defend it.") is the reason the floor exists. Holding `strip` confers nothing for classification, titling, conversation, or the partner calls; passing another structured task confers nothing for the strip.
