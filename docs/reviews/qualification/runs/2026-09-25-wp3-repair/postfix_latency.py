...
OUT = Path(sys.argv[1]) if len(sys.argv) > 1 else HERE / "postfix-latency.json"
...
def one_trial(index: int) -> dict:
    ...
    def submit(content, conversation_id, *, on_delta=None, merged=False):
        return deliberated_send(
            ...
            on_delta=on_delta,
            # The production voice door: a spoken turn is sealed, so it makes no
            # classifier and no strip call (WP3 §2.3).
            spoken=True,
        )
