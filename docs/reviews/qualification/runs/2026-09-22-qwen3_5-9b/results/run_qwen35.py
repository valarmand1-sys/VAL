"""Qwen3.5-9B against the two frozen visual cases, run once each.

Case A uses the genuine Track C attachment and the repaired Case A criterion.
Case C uses the repaired fixture frozen at 53705f2. Both digests are checked
before inference — a run against the wrong bytes would prove nothing.

Generation is untuned: the library defaults frozen in
GENERATION_CONFIGURATION.md (committed at f0c81dd, before this ran), nothing
passed to steer sampling, nothing changed between the cases. The resolved
settings are echoed into the report so the record shows what actually governed
...
MODEL = "/Users/josepharmand/.val-models/Qwen3.5-9B-MLX-4bit"
FIX = Path(
...
def main() -> int:
    ...
    from mlx_vlm.generate import common as gen_common
    ...
    # The settings that actually govern, read back from the runtime rather than
    # asserted: nothing is passed to generate(), so these are what apply.
    report["generation_settings_in_force"] = {
        "max_tokens": gen_common.DEFAULT_MAX_TOKENS,
        "temperature": gen_common.DEFAULT_TEMPERATURE,
        "top_p": gen_common.DEFAULT_TOP_P,
        "top_k": gen_common.DEFAULT_TOP_K,
        "min_p": gen_common.DEFAULT_MIN_P,
    ...
    if which in ("a", "both"):
        ...
        found = digest(image)
        report["case_a_source"] = {
            "path": str(image),
            "sha256": found,
            "matches": found == IMAGE_DIGEST,
        }
