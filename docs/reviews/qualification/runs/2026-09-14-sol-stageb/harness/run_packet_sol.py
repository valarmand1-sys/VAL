"""Run the frozen packet v1.6 on the GPT-5.6 Sol candidate through the candidate lane.

Usage: run_packet_sol.py <out.json> <cap_usd>

Stage B, owner-authorised 14 September 2026. The v1.6 harness with exactly the
adaptation the ruling permits: no registry substitution — the candidate lane
(`candidate_gateway_for_scratch_store`, `deliberate.send(candidate=sol)`) runs
`gpt-5-6-sol-medium` NOT_ADMITTED, under persona v1.8, for the partner-class
calls only; classification and strip route as always. Rows are selected by
exchange identity and the measurement sidecar. A cap ledger refuses, before any
provider call, a reservation that would take cumulative settled spend plus the
call's maximum over the authorisation, and the run stops there. Corpus, order,
seeds, mechanical checks and packet failure semantics are unchanged.
"""
...
from corpus_v16 import ACCESS, CONSEQUENTIAL, INSTRUCTION, L1_QUESTION, O10_HISTORY, ORDINARY, TRAPS, long_note
from alembic import command
from alembic.config import Config
from sqlalchemy import create_engine, text
ROOT = Path("/Users/josepharmand/Projects/val"); URL = "postgresql+psycopg://localhost:5433/val_test"
EFFORT = "medium"; OUT = Path(sys.argv[1]); CAP_USD = float(sys.argv[2])
...
from val_domain.gateway import CacheTtl, ModelConfig, TaskType, TurnReference
...
# --- the exact configuration under test: the registry's own candidate entry, untouched ---
config = by_slug("gpt-5-6-sol-medium"); assert config is not None
assert config.admission.value == "not_admitted" and not config.capability_profiles and config.fallback_slug is None
assert config.reasoning_effort.value == "medium" and config.model_identifier == "gpt-5.6-sol"
assert {t.value for t in config.qualification_targets} == {"partner"}
CONFIGURATION = {"provider": config.provider, "model": config.model_identifier, "slug": config.slug, "id": str(config.id),
                 "api_mode": "Responses API, streamed; strict json_schema on the blind call", "effort": config.reasoning_effort.value,
                 "admission": config.admission.value, "capability_profiles": [], "qualification_targets": ["partner"],
                 "cache": "automatic (provider); verified $5 write / $0.40 read", "partner_output_ceiling": 4096,
                 "lane": "candidate_gateway_for_scratch_store; deliberate.send(candidate=sol)"}


class StageBCapReached(Exception):
    """Raised inside the ledger before a provider call the authorisation cannot cover."""


class CapLedger(DatabaseLedger):
    """The real ledger, plus the owner's Stage B authorisation enforced before each call."""
    ...
    def reserve(self, config: ModelConfig, max_cost_usd: float, task_type: TaskType, project_id, exchange: TurnReference | None = None):
        ...
        if settled + max_cost_usd > CAP_USD:
            raise StageBCapReached(json.dumps({"cumulative_settled": round(settled, 6), "next_call_max": round(max_cost_usd, 6),
                                              "task_type": task_type.value, "cap": CAP_USD}))
...
adapters, problems = build_adapters({"anthropic", "openai"}); assert not problems, problems
gateway = candidate_gateway_for_scratch_store(
...
with engine.connect() as c:
    _p = c.execute(text("select semantic_version, source_sha256 from personas where id = :i"), {"i": persona_id}).one()
CONFIGURATION["persona_semantic_version"] = _p.semantic_version; CONFIGURATION["persona_source_sha256"] = _p.source_sha256
CONFIGURATION["persona_note"] = "scratch-store row seeded from docs/baselines/03-persona.md; identical by digest to the live revision 7 (id 01a09d3b-1870-753d-b0b9-b21728e7e0a8), v1.8"
assert _p.semantic_version == "1.8", _p.semantic_version
assert _p.source_sha256 == "1608715f6cba71059c69d132918838019ffb73c0f11d833b50d2a8156ed9873e", _p.source_sha256
import hashlib
from corpus_v16 import corpus_document
_frozen = (ROOT / "docs/reviews/qualification/corpus/v1.6/corpus.json").read_bytes()
_doc = json.loads(json.dumps(corpus_document(), ensure_ascii=False, default=str)); _fz = json.loads(_frozen)
# The frozen file also carries `status`, `changes_from_v1.5` and `persona_revision` (v1.4, the
# persona the packet was frozen under); every prompt-bearing key must be identical.
assert all(_fz[k] == _doc[k] for k in _doc), "harness corpus differs from the frozen corpus"
assert _fz["version"] == "v1.6"
CONFIGURATION["corpus_sha256"] = hashlib.sha256(_frozen).hexdigest()
CONFIGURATION["packet_sha256"] = hashlib.sha256((ROOT / "docs/reviews/qualification/VAL_Partner_Qualification_Packet_v1.6.md").read_bytes()).hexdigest()
...
def rows_for(user_message_id, conversation_id, since):
    """Every model call this turn made, selected by exchange identity (13 September 2026) with
    its measurement row: Sol's automatic-cache figures live there, not in model_call_cache_usage."""
    with engine.connect() as c:
        calls = [dict(r._mapping) | {"created_at": str(r._mapping["created_at"])} for r in c.execute(text(
            "select m.id::text as call_id, m.created_at, m.task_type::text as task, m.model_config_id::text as config_id, m.model_identifier, m.tokens_in, m.tokens_out, m.cost::float as cost, m.latency_ms, m.terminal_state::text as terminal, m.persona_id::text as persona_id, "
            "x.streamed, x.first_text_ms, x.text_output_chars, x.reasoning_present, x.reasoning_output_tokens, x.provider_cached_input_tokens as read, x.provider_cache_write_tokens as written, "
            "r.max_cost::float as reserved_max, u.outcome as cache from model_calls m join model_call_measurements x on x.model_call_id = m.id "
            "left join budget_reservations r on r.model_call_id = m.id left join model_call_cache_usage u on u.model_call_id = m.id "
            "where x.exchange_message_id = :mid order by m.created_at"), {"mid": user_message_id}).all()]
...
def run_prompt(pid, content, scope, catalogue, conversation_id=None):
    ...
    if conversation_id is None:
        outcome = send(engine, gateway, content, catalogue=catalogue, signals=ProjectSignals(explicit_selection=scope.project.name), candidate=config)
    else:
        outcome = send(engine, gateway, content, catalogue=catalogue, conversation_id=conversation_id, candidate=config)
    ...
    rec = {"id": pid, "prompt": content if len(content) < 2000 else content[:200] + " …[fixed note]", "wall_ms": wall, "outcome": type(outcome).__name__,
           "logs": [l for l in captured[before:] if "history selection" in l or "blind position payload" in l or "prompt cache" in l or "strip" in l or "prior record state" in l]}
    ...
    print(f"[{time.strftime('%H:%M:%S')}] sol-{EFFORT} {pid:<4} {rec['outcome']:<15} wall={wall:>6} cost=${rec.get('turn_cost', 0):.4f} {(rec.get('answer') or rec.get('error') or '')[:70]!r}", flush=True)
    return rec

# --- mechanical checks ---
from val_policy.words import count_words  # ruled word-count definition, 9 September 2026
def words(t): return count_words(t or "")
def check_I(pid, a):
    a = a or ""
    ...
    if pid == "I4":
        # v1.4 criterion, mechanical part: exactly two top-level headings, Cast then Weather; nothing
        # before the first or after the second section (no rule, no sign-off, no address line); at
        # least one line under each. Grounding (risks supported by the supplied facts; no unsupported
        # production facts) is a reading judgment and is left to the reader.
        heads = [i for i, l in enumerate(lines) if re.match(r"^\s*(#+\s*|\*\*)?(Cast|Weather)(\*\*)?\s*:?\s*$", l)]
        names = [re.sub(r"[^A-Za-z]", "", lines[i]) for i in heads]
        first_is_head = bool(lines) and bool(heads) and heads[0] == 0
        ordered = names == ["Cast", "Weather"]
        under_each = len(heads) == 2 and heads[1] - heads[0] > 1 and len(lines) - heads[1] > 1
        ...
        return (len(heads) == 2 and ordered and first_is_head and under_each and nothing_else), f"{len(heads)} headings {names}; first line heading={first_is_head}; content under each={under_each}; nothing else={nothing_else}"
    if pid == "I5":
...
def check_C(rec):
    ...
    return {"blind_row": bool(blind), "blind_enforced": bool(blind) and blind[0]["ordering"] == "enforced", "deliberation_row": bool(delib),
            ...
            "measurement_rows": all(c.get("written") is not None for c in calls if c["task"] in ("blind_position", "conversation")),
            "strip_states": rec.get("strip_states")}

results = {"configuration": CONFIGURATION, "corpus": "v1.6", "cap_usd": CAP_USD, "started": time.strftime("%Y-%m-%d %H:%M:%S"), "prompts": {}}
...
def export_store(out_dir):
    ...
    with engine.connect() as c:
        for t in ["projects", "conversations", "messages", "classifications", "model_calls", "model_call_cache_usage", "model_call_measurements", "budget_reservations", "blind_positions", "deliberations", "personas"]:
...
def stop_on_cap(exc: StageBCapReached, pid: str):
    with engine.connect() as c:
        settled = float(c.execute(text("select coalesce(sum(cost), 0) from model_calls")).scalar_one())
    results["stopped"] = {"at_prompt": pid, "completed_prompts": len(results["prompts"]), "cumulative_settled": settled, "blocked": json.loads(str(exc))}
    results["finished"] = time.strftime("%Y-%m-%d %H:%M:%S")
    OUT.write_text(json.dumps(results, indent=2, ensure_ascii=False, default=str))
    export_store(str(OUT.with_suffix("")) + "_store")
    print("STOPPED at cap:", results["stopped"], flush=True)
    sys.exit(2)
...
except StageBCapReached as _cap:
    stop_on_cap(_cap, current_pid)
