"""The real service, on a scratch store, a second port and an ISOLATED model — measurement only.
...
#: Priming-cache pass, 25 September 2026. The experiment's model instance: the
#: admitted artifact cloned under another LM Studio key and loaded as `val-exp`, so
#: this process can never address production's instance, nor production this one.
#: Only this process's registry changes; nothing on disk does.
EXPERIMENT = os.environ.get("VAL_SCRATCH_MODEL_IDENTIFIER", "val-exp")
PARTNER = "gpt-oss-20b-mxfp4-mlx-lmstudio-partner"
registry.REGISTRY = tuple(
    config.model_copy(update={"model_identifier": EXPERIMENT}) if config.slug == PARTNER else config
    for config in registry.REGISTRY
)
#: Cold-model condition A: load the experiment as production loaded before this pass.
if "VAL_SCRATCH_PARALLEL" in os.environ:
    import val_providers.lmstudio_runtime as lmstudio_runtime

    lmstudio_runtime.SERVING_PARALLEL = int(os.environ["VAL_SCRATCH_PARALLEL"])
#: Condition B of the matrix: sequential serving with priming switched off.
if os.environ.get("VAL_SCRATCH_PRIMING", "on") == "off":
    Gateway.prime_prefix = lambda self, still_wanted=None: {  # type: ignore[method-assign,assignment]
        "primed": False,
        "outcome": "skipped",
        "reason": "priming off for this measurement condition",
    }
