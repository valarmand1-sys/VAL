...
adj = lambda d: d["prompts"].get("I1", {}).get("mechanical_adjudicated")
if adj(cand) or adj(inc):
    mech.append(f"| I1 adjudicated (ruled lexical count, 9 Sep 2026) | {adj(cand)} | {adj(inc)} |".replace("'ruling':", "'ruling':"))
ann = lambda d: d["prompts"].get("I1", {}).get("word_count_annotation")
if ann(cand) or ann(inc):
    mech.append(f"| I1 word counts (as run / lexical) | {ann(cand)['as_run_count']} / {ann(cand)['lexical_count']} {ann(cand)['standalone_punctuation']} | {ann(inc)['as_run_count']} / {ann(inc)['lexical_count']} {ann(inc)['standalone_punctuation']} |")
