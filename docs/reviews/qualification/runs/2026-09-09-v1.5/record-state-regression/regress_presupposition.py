"""Bounded behavioural regression: the unsupported-presupposition class, on one configuration,
through the real gateway with the prior-record-state envelope (ruling, 9 September 2026).

Usage: regress_presupposition.py <effort> <samples> <out.json>

Validation of the architectural fix, not a run intended to manufacture a pass. Each case runs in
its own empty isolation project (nothing to recall), N samples each, ordinary turns only. The
answers are returned for reading; a lexical screen marks candidate continuity markers only as
an aid, never as a verdict.
"""
...
# The class: prompts whose wording presupposes shared context the record does not hold.
CASES = {
    ...
    "P1":  "Pick up where we left off on the harbour scene.",
    "P2":  "Draft a two-line reply to the costume designer's last email.",
    "P3":  "Remind me what you thought of the workshop pages.",
    "P4":  "Give me the note for the AD about the schedule change we discussed.",
    "P5":  "What's still open from yesterday?",
    "P6":  "Write the one-line message to Faye confirming the read-through room.",
}
MARKERS = re.compile(r"\b(again|last (time|week|season|email|review)|yesterday|earlier|previously|as (we|you) (said|agreed|discussed)|four times|third|fourth|the (same|usual)|we (settled|agreed|decided)|you('ve| have) been|putting off|second volume|volume on)\b", re.I)
...
for cid, content in CASES.items():
    for i in range(1, N + 1):
        catalogue, scope = project(f"R-{cid}-{i}")
        ...
        rec = {"case": cid, "sample": i, "prompt": content, "wall_ms": wall, "prior_record_state": json.loads(state) if state else None,
               "answer": answer, "error": error,
               "lexical_markers": sorted({m.group(0).lower() for m in MARKERS.finditer(answer or "")})}
        results["cases"].append(rec)
        print(f"{cid} #{i} wall={wall:>6} markers={rec['lexical_markers']} {(answer or rec['error'])[:100]!r}", flush=True)
