"""Strip conformance suite v4 through the real gateway (rulings, 11 September 2026).

Usage: run_strip_v4.py OUT.json CAP_USD CASES route:runs [route:runs ...]
  CASES  comma-separated case ids, or "all"
  route  a registry slug; evaluation-only entries (NOT_ADMITTED) go through the
         gateway's evaluation door, admitted ones through the pinned path

Each call runs `val_gateway.deliberate._strip` pinned to the exact registered
configuration in the scratch store — reservation, the completeness guard and
retry rule, the record-evidence grounding against the case's
`history_val_messages`, the model_calls row and the settlement, all as live.
After every call the cumulative measured cost is read back from model_calls;
the run stops when it exceeds CAP_USD.
"""
...
from val_policy.deliberation import _QUOTE_TRIM, _collapse, same_text
...
suite = json.load(open(ROOT / "docs/reviews/qualification/strip-conformance/v4/suite.json"))
assert suite["frozen"] is True, "v4 must be frozen before it is run"
cases = [c for c in suite["cases"] if case_filter is None or c["id"] in case_filter]


def norm_quote(t):
    return _collapse(t).casefold().strip(_QUOTE_TRIM)
...
def judge(case, attempts):
    ...
    exp = case
    problems = []
    declared = sorted(norm_quote(q.text) for q in (attempts.outcome.record_evidence if attempts.outcome else ()))
    expected_evidence = sorted(norm_quote(e["text"]) for e in exp.get("expected_record_evidence", []))
    ...
    if v.state == "not_separable" and exp.get("accept_not_separable"):
        return "conformant", ["separable=false accepted for this case"]
    if not exp["expected_spans"]:
        if v.state != "no_preference":
            return "BLOCKING" if v.state == "enforceable" else "false-contamination", [f"expected no_preference, got {v.state}: " + "; ".join(v.reasons)]
        if declared != expected_evidence:
            return "BLOCKING", [f"record evidence declared {declared} != expected {expected_evidence}"]
        return "conformant", []
    if v.state == "enforceable":
        ...
        for forbidden in exp.get("forbidden_in_residue", []):
            if forbidden.casefold() in (v.residue or "").casefold():
                problems.append(f"forbidden content in residue: {forbidden!r}")
            if any(forbidden.casefold() in d for d in declared):
                problems.append(f"forbidden content declared as record evidence: {forbidden!r}")
        if declared != expected_evidence:
            problems.append(f"record evidence declared {declared} != expected {expected_evidence}")
...
if os.environ.get("RESUME_FROM"):
    # Resume: keep the (route, case, run) triples already measured in an
    # earlier partial run, count their measured cost toward the cap, and run
    # only what is missing. Evidence is never re-bought.
    results = json.load(open(os.environ["RESUME_FROM"]))
    prior_spend = sum(r["cost"] for r in results)
    print(f"RESUMING from {len(results)} recorded calls, ${prior_spend:.4f} already spent", flush=True)
done = {(r["route"], r["case"], r["run"]) for r in results}
stopped = None
for slug, runs in plan:
    config = by_slug(slug)
    assert config is not None, slug
    evaluation = config.admission.value == "not_admitted"
    for case in cases:
        for run in range(1, runs + 1):
            if (slug, case["id"], run) in done:
                continue
            with engine.connect() as c:
            ...
            attempts = _strip(
                ...
                configuration=config, evaluation=evaluation,
                record=tuple(case.get("history_val_messages", [])),
            )
            ...
            rec = {
                ...
                "record_evidence": [{"text": q.text, "occurrence": q.occurrence} for q in (attempts.outcome.record_evidence if attempts.outcome else ())],
            ...
            if total + prior_spend > cap:
                stopped = f"STOPPED: cumulative measured cost ${total + prior_spend:.4f} exceeded cap ${cap:.2f}"
...
for slug, _ in plan:
    ...
    by_case = {}
    for r in rs:
        by_case.setdefault(r["case"], []).append(r["verdict"])
    for cid, vs in by_case.items():
        print(f"   {cid:<4} conformant {vs.count('conformant')}/{len(vs)}  false-contamination {vs.count('false-contamination')}  BLOCKING {vs.count('BLOCKING')}")
with engine.connect() as c:
