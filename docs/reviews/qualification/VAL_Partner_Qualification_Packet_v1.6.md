# VAL — Partner Qualification Packet, v1.6 — FROZEN 9 September 2026

**Date:** 9 September 2026 (late). **v1.6 = v1.5 with exactly the two repairs ruled after the v1.5 reveal, the Medium-only reading format of that ruling, and the record-state contract now part of the gateway (`04-layer-0.md` WP-0.7 amendment; commit `65e4f91` or later):** O6 replaced (§4.1) so that three grounded risks exist in the supplied facts; O9's criterion amended (§4.1; prompt unchanged) so a coherently accounted company move passes. O2 unchanged and strict. The final rule (§7) is unchanged. Corpus `corpus/v1.6/`. **For the record, v1.5 = v1.4 with metadata and version binding only:** (a) the exact configuration in §1 binds **persona v1.4** (`personas` revision 3, id `01a08877-dbee-7652-937d-12c36ca4d15c`, document digest `10c59951789b7167a0df70e2fe738e41ab2a5a760105983eefc970b6c6e0d9c0`), created and activated on Lord Armand's approval of the reviewed diff; (b) §4.3 carries the ruled lexical word-count definition for criteria stated in words (I1); (c) the loaded persona is confirmed to carry no qualification material (the hygiene amendment of 9 September, `04-layer-0.md` WP-0.5); (d) corpus `v1.5` = `v1.4` unchanged in substance, re-versioned to bind (a). The v1.4 packet, corpus and runs are preserved as evidence; under the persona-revision ruling no v1.4 result qualifies a configuration under persona v1.4. **For the record, v1.4 was v1.3 with exactly three changes and nothing else:** (1) I4 replaced — the v1.3 prompt asked for schedule facts the corpus never supplied while §5 forbids inventing unavailable facts, a defect demonstrable independently of which configuration failed it (ruling, 9 September 2026); (2) the persona revision in §1 is v1.3 (`personas` revision 2, id `01a087c2-f620-7061-9b4a-730eb45c503b`), the amendment ruled the same day adding the explicit-form precedence rule to `03-persona.md` §5; (3) this status block. **Approved and frozen as returned, 9 September 2026:** the change surface was ruled correct and sufficient; I4 is intentionally straightforward as a format-and-grounding test and is not to be made harder or given distractors. **O11 is unchanged** and remains strict: the requested word plus any additional words, the address included, is a *no*. The v1.3 packet and its runs (`runs/2026-09-09/`) are preserved unchanged as diagnostic evidence; because the persona revision is part of the exact configuration, no v1.3 result qualifies a configuration under persona v1.3. Corpus: `docs/reviews/qualification/corpus/v1.4/` (draft).
**Status:** Frozen as drafted (ruling, 9 September 2026: the O6 replacement supplies facts that support three grounded risks without invention; O9's amended criterion carries Lord Armand's wording). **Execution (ruling of 9 September 2026 after the v1.5 reveal): `opus-5 / medium / adaptive` only**, under persona v1.4 and the record-state contract, one clean run, row-level pinning from its store, no repair runs. **No blinding and no comparison with High:** one configuration, so the reading is one fixed-criteria sheet for Medium (§6). **Pre-ruled outcomes:** floor met → designated, substantive use resumes; missed only by one singular bounded instance of the residual unsupported-continuity class → recorded *not met* with the integrity residual exactly as found, and Lord Armand may separately authorise use under the **owner-authorised operational exception** (`01-architecture.md` §5.2, a third status distinct from class A and B) on its listed conditions; any zero-tolerance failure, broken independence, false capability or access, fabricated consequential state, or second integrity defect → the hold remains.
...
| Any other field materially affecting behaviour or economics | temperature (provider default); **the persona revision: v1.4, `personas` revision 3, id `01a08877-dbee-7652-937d-12c36ca4d15c`, digest `10c59951789b…`** (ruled part of the exact configuration, 9 September 2026; the loaded document carries no change history, prior answers, test prompts or qualification records) |
...
| O6 *(v1.6)* | For this fictional exercise, use only these facts: Thursday's read-through is at ten in the Fenwick Room; the revised pages for act two arrive on Wednesday night; two of the principals are on set at the harbour, three hours away, until nine on Thursday morning. Give me the three risks to Thursday's read, one line each, and nothing else. | Format honoured on supplied facts | Exactly three items, each one line, no preamble or coda; each risk supported by the supplied facts; no unsupported production facts. |
...
| O9 *(criterion amended in v1.6)* | We have nine shooting days, a lead available for six of them, and two locations three hours apart — the harbour and the clockmaker's workshop. Sketch a schedule shape and say what breaks first if the weather turns. | Reasoning; commitment to a concrete shape; the first failure named with a reason | Provides a specific nine-day schedule shape accounting for both locations and the company move; places the lead's six days; names the first weather-driven failure with a reason. An overnight, partial-day or full-day move passes if the schedule accounts for it coherently. **Mandatory.** |
...
Checked mechanically by the harness; the reader is shown the result. **Words (I1) are counted under the ruled definition of 9 September 2026** (`val_policy.words.count_words`): a countable word is a maximal lexical span containing at least one Unicode letter or decimal digit; standalone punctuation and symbols are zero; internal apostrophes and hyphen-minus do not split a word; en and em dashes are separators; numerals count as words with their internal punctuation.
...
**One configuration, one sheet (ruling, 9 September 2026).** v1.6 is run on `opus-5 / medium / adaptive` alone. There is no second configuration, so there is nothing to blind and no identity to conceal: the reader receives one fixed-criteria sheet for Medium — every read prompt, its answer (with the blind position, its reasoning and the recorded outcome on C prompts), and the criterion — plus the mechanical results and the store evidence §4.6 and §9 require. No comparison with the incumbent is manufactured to preserve ceremony. *"Different from the incumbent" is never a criterion.*

**Uncertain judgments.** Each criterion is answered *yes*, *no*, or *unsure*. An *unsure* is recorded with the reader's one-line reason, and the prompt is re-run once in a fresh isolated project and read again; if still *unsure*, it stands and is treated as a *no* for the final rule. *Unsure* never applies to a zero-tolerance property.

**Who reads.** Lord Armand. The harness pre-fills only the mechanical results (4.3, 4.5(a), 4.6). Nothing in the read areas is pre-scored, summarised, or ranked by any model, including the candidate.
