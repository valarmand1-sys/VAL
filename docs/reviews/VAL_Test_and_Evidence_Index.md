...
## 69. First local cognition provider — LM Studio, evaluation only — 16 September 2026

Owner rulings of 16 September 2026 (`04-layer-0.md` §5 amendment; `01-architecture.md` §5.4). **Built:** the `hosting` axis and `LOCAL_NO_METERED_COST` metering with validators (a zero rate is refused on every metered route); `val_providers.lmstudio_adapter` — loopback-only, token required to construct and never shown, chat-completions dialect independent of the cloud Responses adapter, conversation-only, no tools, no client retries, hidden reasoning discarded at the boundary in both modes, model-echo mismatch refused, a prompt that filled the loaded context refused; startup wiring (`VAL_LMSTUDIO_BASE_URL`, `VAL_LMSTUDIO_API_TOKEN` — required where the adapter is built, not by the production service while no local route is admitted); registry entry `gpt-oss-20b-mxfp4-mlx-lmstudio`, `NOT_ADMITTED`, no profile, PARTNER target for the candidate lane only, eligible for the incumbent's non-Restricted set; unmetered settlement at a known $0 whether or not usage was reported (two defects found and fixed on the way: the budget bound and settlement divided by the base rate; the `model_calls` known-cost check required tokens — widened by exactly one provider-named clause, migration `0022`); runtime provenance (`provider_reported_model`, `runtime_diagnostics` with the hosting axis); the scope tripwire's expiry made precise. **Ruling 3a answered:** on a $0 route nothing monetary refuses a call; the number of provider calls per exchange is bounded by the orchestration (≤ 4 for a consequential exchange, 2 for an ordinary one, no retry on truncation, no fallback on the lane, SDK retries off) — pinned by tests; no new guard needed at Layer 0. **Tests:** 37 adapter, domain/policy/gateway/startup/tripwire suites; full mirror green. **Live loopback proof** (`docs/reviews/qualification/runs/2026-09-16-lmstudio-loopback/`, candidate lane, scratch store, $0): COLD T1 first chunk 13.45 s (JIT load ≈ 6.1 s + prefill ≈ 7.4 s), first visible text 16.13 s, total 21.88 s; WARM T1 first chunk 7.38 s (prefill ≈ 734 tok/s), first visible 14.01 s, ≈ 62 tok/s generation, total 18.82 s; prompt 5,417 tokens; reasoning tokens reported; model echo matched; every row `$0 KNOWN` with full provenance. **Limitations found:** LM Studio's JIT reload restores the model's *default* context (8,192), not the 32,768 of a manual load; the house byte bound (≈5× the real count) refused the second turn even at 32,768. **Reconciled later the same day:** the owner keeps the model loaded as the single canonical instance at 32,768 and ruled that the verified runtime state; the entry now carries 32,768 with the JIT hazard recorded. At that window the byte bound refuses the *consequential response call on its first turn* (≈27.1k + 6,144 > 32,768) — pinned as evidence (`test_local_route_accounting.py`); the proof was not rerun and the preflight was not changed, because changing it contradicts the 8 September 2026 ruling that placed the byte bound in the context-window preflight — stopped for ruling. **Not done:** no admission, no profile, no routing change, no Restricted eligibility, no benchmark, no preflight change.

## 70. Local context preflight — exact tokenization through the read-only LM Studio SDK inspector — 16 September 2026

Owner rulings of 16 September 2026 (`04-layer-0.md` §5, the later amendment of that date): the cloud byte bound unchanged; the local preflight exact. **Built:** `val_domain.provider.ContextFeasibility` / `ContextInspectingAdapter` / `ContextInspectionUnavailableError` (the provider-neutral contract — the core never imports the SDK); `val_policy.budget.local_context_overrun` (`prompt + requested reserve ≤ loaded context`, the cap still refused rather than clamped, the message naming both figures and that nothing was shortened); `val_providers.lmstudio_inspector.LMStudioContextInspector` — `lmstudio==1.6.0b1` pinned exactly by owner exception (1.5.0 cannot authenticate to a token-protected server), a scoped `Client(api_host, api_token=…)` with the house token passed programmatically (never `configure_default_client`, never the environment fallback), loopback hosts only, `list_loaded` only (never `llm.model()`, never load/unload), deterministic single-instance selection or refusal, `apply_prompt_template` + `count_tokens` + `get_context_length` on the loaded handle, version and instance provenance in the result; the adapter's `measure_context` on exactly the turns `_request` transmits; startup wiring of the inspector beside the adapter; the gateway's `_attempt` measuring on `LOCAL_NO_METERED_COST` routes with the runtime authoritative (registry disagreement logged and recorded), falling closed to the byte bound on `ContextInspectionUnavailableError`; `preflight` and `parity` blocks on `runtime_diagnostics` (the SDK count beside the server's `usage.prompt_tokens`, `exact` true or false, a warning when not); `CandidateGateway.measure_candidate_context` and the deliberation path's early feasibility check (`_known_material_cannot_fit`) refusing a consequential exchange before its classification call when the known material cannot fit — classification, strip, blind position and response all skipped, nothing shortened. **Pins moved under the ruling, and say so:** the 13 September "no runtime source names a token-counting endpoint" pin now forbids provider counting endpoints everywhere and names the one module permitted to count — the local inspector — as the ruled exception. **Tests (A–O):** inspector boundary (no generative or load-bearing method; `list_loaded` only; zero/ambiguous/wrong-identity/missing-context/unreachable all refuse; the credential passed once and rendered nowhere; measurement on the exact turns with the SDK's `Chat` merging recorded as the SDK's behaviour); policy inequality at the boundary, runtime-over-registry, cap refusal, cloud byte bound untouched; gateway preflight at 8,192 vs 32,768 on the same prompt, unavailable → byte bound, cloud routes never measured, the reserve boundary, parity rows exact and inexact, early refusal before classification, the full consequential sequence when it fits, post-blind refusal when it does not; adapter measurement unavailable without an inspector and never estimated. Focused suites and the full mirror green. **Live proof (candidate lane, scratch store, $0; `docs/reviews/qualification/runs/2026-09-16-lmstudio-loopback/README.md`, "Exact preflight and parity"):** at the ruled runtime check the machine had just rebooted (≈19:05 CDT) and the instance was gone — nothing was loaded by the house; the owner reloaded it and the check then found exactly one resident instance at 32,768 by `lms ps`, the native listing and the SDK inspector alike, still resident after the run. **Parity exact on every admitted call:** T1 5,417 = 5,417, T2 5,769 = 5,769, T3 6,046 = 6,046 (SDK count = server `usage.prompt_tokens`, difference 0, `registry_agrees` true, no warning). **The three-turn exchange the byte bound had refused from T2 completes at 32,768:** first visible text 10.42 / 11.29 / 15.32 s, totals 15.5 / 15.3 / 17.4 s, ≈62 tok/s, every row `$0 KNOWN` with full provenance (LM Studio 0.4.24+1, SDK 1.6.0b1, MXFP4/MLX, loaded 32,768 of 131,072, reasoning medium). Parity demonstrates agreement for the calls that ran; it does not prove the SDK can never conservatively overcount a request refused before inference. History headroom at this shape: 21,207 tokens after the first turn, growing ≈280–350 per exchange — no collision reached; the forty-message maximum binds first. Early refusal not exercised live (no consequential turn in the harness); pinned by tests. **Not done:** no admission, no profile, no routing change, no Restricted eligibility, no cloud preflight change, no benchmark.

## 71. GPT-OSS Partner-quality qualification — Stage A, the frozen local benchmark — 16 September 2026

Owner phase ruling of 16 September 2026: a $0 local quality benchmark to decide whether GPT-OSS at medium reasoning earns a paid head-to-head against Sol. **Frozen before inference at `60b65b7`:** twelve tasks, two per area (persona fidelity, instruction following, writing/editing, reasoning/planning, honesty/boundaries, multi-turn correction), representative not adversarial, mechanical checks frozen with each task. **Run** through the real candidate path on the scratch store (persona v1.8 whole, Core envelopes, retained history, exact preflight, HTTP inference; no classification/strip/blind call) against the verified single resident instance at 32,768 (LM Studio 0.4.24+1, MXFP4/MLX, SDK 1.6.0b1 for inspection only), unchanged throughout. **Mechanical outcome:** 16/16 turns answered, no refusal, truncation or runtime error; parity exact on every call; median first visible 13.2 s, median total 16.3 s; cloud spend $0, local $0 KNOWN. Constraints all met on 8/12 (three of the four misses are harness-check artifacts, one a genuine violation — F2 named the river after being told not to); engineer's read: invention 3/12 (A1 invented prior practice and project history; D2 a weekday and 'proven capacity', plus an arithmetic error on the four-week window; E2 an invented system-log observation), material omission 2/12 (B2's empty closing sentence; F2's missed insurance risk), contradiction 2/12 (F1 signed the user's invitation as Val; F2 moved the Thursday wind to Tuesday), corrections preserved on both multi-turn tasks. Persona notes for the owner: markdown tables and headers on analytical answers, codas and sign-offs, American spellings. **Artifacts:** `docs/reviews/qualification/runs/2026-09-16-gpt-oss-stage-a/` — `review-packet.md` is the owner review packet. **Not declared:** pass, fail, comparison, admission. **Not done:** no Sol comparison, no admission, no profile, no routing change, no low-reasoning variant, no other local model.

## 72. Qwen3.8-27B challenger — registered for evaluation, STOPPED at the ruled gates — 16–17 September 2026

Owner rulings of 16 September 2026 (capability-gap review accepted; the one challenger `lmstudio-community/Qwen3.8-27B-MLX-6bit`, medium effort, no sampling override) and 17 September 2026 (MLX auto-fit exception: configured 32,768, actual 80,896 recorded distinctly, actual governing). **Done:** the artifact obtained (22,804,836,386 bytes); LM Studio's canonical identifier `qwen3.8-27b-mlx` read from its own listings; registry entry `qwen3-8-27b-mlx-6bit-lmstudio` NOT_ADMITTED, no profile, PARTNER target only, local unmetered, MEDIUM, with pinning tests and three enumeration pins moved under the ruling; the Stage A harness given a candidate override that leaves the frozen task set untouched; a concrete defect fixed (the pins check flagged a word inside a recorded GPT-OSS answer — run evidence now excluded from the placeholder scan on the same grounds as economics evidence); full mirror green; commit `5801f11`. **STOPPED at the gates (run README `docs/reviews/qualification/runs/2026-09-16-qwen3.8-27b-stage-a/`):** (1) the adapter transmitted `reasoning_effort: medium` and LM Studio rendered the model's system line as *"Reasoning effort is set to xhigh"* — the runtime does not apply the transmitted effort to this template, so the request ran at the preset's default; (2) parity SDK 5,560 vs server 5,561, +1 — tokenization of LM Studio's logged input agrees exactly, the difference is the separator each side inserts when merging the two consecutive user messages (server a blank line, SDK none). Nothing compensated, no benchmark prompt sent. The one call at `xhigh`: first visible 147.82 s, total 180.454 s, 1,046 reasoning tokens — not Stage A evidence. Cloud $0. Qwen and GPT-OSS remain NOT_ADMITTED; Sol remains the production Partner.

## 73. Local wire canonicalization — exact preflight and inference consume one canonical sequence — 17 September 2026

Owner ruling of 17 September 2026 (`04-layer-0.md` §5 amendment): first seam fixed, exact preflight / serialization parity; reasoning-control declaration deferred. **Root cause in code and tests:** Core structurally assembles the record-state envelope and the current turn as two adjacent user messages; LM Studio's SDK `Chat` merged them into content parts (a Jinja template concatenates with nothing) while its chat-completions ingress merged them with a blank line — 5,560 vs 5,561 on Qwen3.8-27B with tokenization agreeing (§72). **Built (`359bc25`):** `canonicalize_turns` in the LM Studio adapter — consecutive same-role messages become one message of that role joined by exactly one blank line, role-based, model-independent, never across roles, order and text otherwise unchanged, idempotent — applied inside the one construction (`_chat_turns`) both the inspector measurement and the request already shared; Core untouched. Tests A–J (`test_lmstudio_canonicalization.py`) plus the Val shape with and without history; two adapter pins moved under the ruling; GPT-OSS preflight and accounting regressions green; full mirror green. **Proofs (`docs/reviews/qualification/runs/2026-09-17-local-wire-canonicalization/`):** non-inference render on the resident Qwen artifact — old seam empty, canonical seam a blank line equal to LM Studio's own logged ingress, one token apart; live Qwen call under the auto-fit exception — **5,561 = 5,561, exact** (not qualification evidence; the runtime still rendered xhigh); live GPT-OSS regression at 32,768 — **5,417 = 5,417, exact**, unchanged from every earlier GPT-OSS proof. Cloud $0. Qwen remains NOT_ADMITTED and closed; GPT-OSS NOT_ADMITTED; Sol the production Partner. Qwen's artifact has served its purpose as the seam fixture.

## 74. Mistral Small 3.2 24B — the Category-A challenger through the frozen Stage A suite — 17 September 2026

Owner rulings of 17 September 2026. **Built (`111fa5e`, `5374fd2`):** registry entry `mistral-small-3-2-24b-8bit-mlx-lmstudio` under LM Studio's canonical id `mistral-small-3.2-24b-instruct-2506-mlx`, NOT_ADMITTED, no profile, PARTNER target only, local unmetered, `NOT_APPLICABLE` reasoning (Category A verified on template, configs, LM Studio metadata and at runtime), the upstream temperature 0.15 pinned through the existing per-entry temperature and proven on the wire; pins moved and tests added; full mirror green twice. **Artifact:** 25,927,575,926 bytes of weights, every shard SHA-256-verified against the publisher after LM Studio's downloader stalled and was completed. **Gates:** load at a configured 32,768 came up at 36,352 (MLX substitution, accepted as a Mistral-only exception, recorded distinctly); template inspection passed on every required finding; the one proof call exact (5,424 = 5,424) with temperature 0.15 transmitted and no reasoning evidence. **Stage A (`docs/reviews/qualification/runs/2026-09-17-mistral-small-3.2-stage-a/`, `review-packet.md`):** 16/16 answered, no refusal, truncation or error; median first visible 2.98 s and total 17.0 s at ≈10 tokens/s with LM Studio's prompt cache warm; constraints all met 9/12; invention 2/12, omission 3/12, contradiction 2/12, corrections preserved 2/2; persona conduct breaches (stage directions, quoted reference lines, third-person self-reference) noted for the owner. **FINDING:** exact preflight parity held on the proof call and all twelve single-turn calls but not on the four multi-turn calls — the preflight over-counts by exactly one token per assistant message in history (−1, −2, −1, −2); read-only diagnosis excludes whitespace and points at how the two LM Studio ingress paths terminate assistant history turns (the template's end token), a hypothesis pending owner ruling; nothing calibrated; every difference conservative. **COMPARABILITY NOTE:** GPT-OSS Stage A ran with no pinned sampling; Mistral's with upstream temperature 0.15 pinned. No pass, fail, comparison or admission is declared. Cloud $0.

## 75. Assistant-history seam closed; the four quarantined Mistral calls rerun exact — 18 September 2026

Owner ruling of 18 September 2026. **Status of the Mistral first run:** partial evidence, not exact-parity clean as a whole; the harness's failure to halt on the first inexact call recorded as a harness-control defect and fixed (shared `parity_halt` gate after every call, CI pin). **Root cause demonstrated** by one synthetic four-message diagnostic through the real adapter path with LM Studio's model log captured: the template RPC renders historical assistant turns with the template's end-of-sequence token, the chat-completions ingress without — one token per assistant message, tokenization agreeing (SDK 32, server 31). **Fix (`31f882e`)** at the local boundary: the inspector renders with the SDK's documented `omitEosToken` option, byte-equal to the ingress, recorded on every row; model-independent (Harmony byte-identical either way; GPT-OSS live two-turn parity exact, 5,417 and 5,743). Frozen mechanical checks moved into one shared module; the rerun script rebuilds each affected call from the original captured turns. Full mirror green. **Reruns (only the four affected calls):** F1 T2 5,633, F1 T3 5,754, F2 T2 6,526, F2 T3 6,831, all exact, temperature 0.15 transmitted, no reasoning; F1 T3 byte-identical to its original; F2 T2 and T3 now name the drone operator's Tuesday-only booking correctly while retaining the Thursday-wind misreading; F1 T2 re-produces the third-person 'unable to draft' preamble. Lineage in the packet; originals untouched. No verdict; Mistral, GPT-OSS and Qwen remain NOT_ADMITTED; Sol remains the production Partner. Cloud $0.

## 76. Mistral Small 3.2 — final disposition; Gemma 4 31B reassessed and authorised for qualification on a second local provider — 18 September 2026

**Mistral Small 3.2 24B Instruct: NOT_ADMITTED AS LOCAL PARTNER, PARTNER QUALIFICATION CLOSED (owner/VAL, 18 September 2026)** — model-quality failure for the Partner role, especially cross-constraint reasoning, epistemic discipline and capability-state interpretation; all evidence of §74–§75 preserved, positive and negative; the registry entry remains as an evaluation-only historical record and the artifact is removed after this record. **Gemma 4 31B:** the fresh serving-contract reassessment (`docs/reviews/qualification/runs/2026-09-18-gemma-4-31b-contract-reassessment/`, commit `eb950b0`) found LM Studio contract-incompatible on thinking governance and the standalone llama.cpp server compatible pending live proof; the owner authorised a second LOCAL provider (`llama.cpp` / `llama-server`, infrastructure only, candidate-only), the Q6_K artifact, an orthogonal binary thinking declaration, `preserve_thinking`, `top_p` and `top_k` in the configuration contract, the official template pinned at `842da3794eaa`, an explicit 32,768 context with no substitution authorised, and thinking-OFF and thinking-ON pre-suite proofs before any Stage A. Machine provenance corrected to the live value, macOS 26.6.2. No model is admitted by any of this; Sol remains the production Partner.

## 77. Gemma 4 31B (Q6_K, llama.cpp, thinking ON) — template gate closed under the one-terminal-LF rule; both proofs passed; frozen Stage A completed at exact parity — 18 September 2026

**Owner ruling, 18 September 2026:** the one-byte terminal-LF normalization is accepted and nothing broader — raw official `ae53464bf3be25802b3a5b37def7fd89667067d7577049b3b2d74c4d8de4c6d4` != active; canonical official (exactly one terminal LF removed) `6a1015c47ccfcfa67c3b772385bccee357a4d37c3cda37bd202e9047f391ab82` == active. The rule is `val_providers.llamacpp_inspector.compare_with_pinned_template`, pinned by seven tests (raw equality, one LF, two LFs, trailing spaces, CRLF, interior change, no terminal LF); LM Studio untouched. **Rendered equivalence (read-only):** the server was launched from the raw pinned file; one harmless canonical body rendered twice per thinking state through `/apply-template`, recorded whole; the dropped byte follows a right-trimming tag; limitation recorded — no second engine rendered the raw file independently. **Security gate closed** with every counter zero. **Registry:** `gemma-4-31b-q6k-llamacpp`, evaluation only, `NOT_ADMITTED`, no profile, `PARTNER` target, known $0, thinking ON, `preserve_thinking` false, 1.0 / 0.95 / 64, context 32,768 (commit `2cb3cc8`). **Proofs** (commit `6c55331`): thinking OFF passed (37 == 37, no reasoning returned); thinking ON two turns passed (5,512 and 5,843 exact; reasoning in the separate field only; persisted message equal to the visible stream; next-turn history equal to the persisted answer). **Stage A:** frozen `60b65b7` suite, sixteen of sixteen calls complete, parity difference 0 throughout, context 32,768 throughout, every frozen mechanical check passed, no hidden-thought marker in any visible answer, local $0 KNOWN, cloud $0; median first visible text 109.4 s, median total 141.1 s; 2,166 of 20,528 completion tokens visible (derived). Evidence: `docs/reviews/qualification/runs/2026-09-18-gemma-4-31b-stage-a/` (`review-packet.md`, `results-stage-a.json`, `observations-engineer.json`, both proof files, `rendered-equivalence.json`, `server-counters-before-first-inference.json`, `reasoning-token-derivation.json`). **No verdict is made.** Gemma, GPT-OSS, Qwen and Mistral remain NOT_ADMITTED; no route changed; Sol remains the production Partner.

## 78. Gemma 4 31B — final disposition — 18 September 2026

**Gemma 4 31B Q6_K, llama.cpp, thinking ON: NOT_ADMITTED AS LOCAL PARTNER, PARTNER QUALIFICATION CLOSED (owner/VAL, 18 September 2026)** — a model-quality and operational-suitability decision from the raw answers of §77: E2 asserted disk exhaustion as the cause of signal 9 without separating evidence from inference; F2 misread Daniel's Wednesday departure condition and later contradicted the dusk requirement for scene 15; D1 found the scene 9 conflict but still placed scene 9 in the Thursday plan; D2 carried overconfident assumptions beside useful calendar reasoning. **Not a failure of** llama.cpp, the `LlamaCppAdapter`, artifact provenance, exact preflight, thinking control, reasoning separation, parity or local-provider infrastructure; every positive technical record of §77 stands. No thinking-OFF quality benchmark, no alternate quantization, no production role, no further Gemma cycle. The registry entry remains as an evaluation-only historical record; the candidate server is stopped and the artifact removed after this record; template, provenance, proofs and raw responses are preserved. No route changed; Sol remains the production Partner.

## 79. Historical cleanup — 18 September 2026

Whole-project read-only audit, then the owner-approved manifest only: `docs/reviews/VAL_Historical_Cleanup_Record_2026-09-18.md`. No source file met the proof standard for obsolete architecture. The fourteen evidence logs cited by §-entries and run READMEs, previously untracked because `*.log` is ignored, are now tracked byte for byte (commit `b45b5d3`, hashes in the record). Gemma artifact removed after the §78 disposition with every evidence file kept. Generated output and the scratch database cleared; `docs/reviews/governing/` (reproducible from the Git commits named in the record) and the stale `VAL_Repo_Manifest.txt` removed; the dead `ReferenceTrust` enum removed. Validation: migration chain intact, focused tests and the full mirror green, desktop rebuilt from the cleaned tree, installed app and authoritative store unchanged. No route changed.

## 80. Live store aligned from 0022 to 0023 — 18 September 2026

Owner instruction, 18 September 2026, after the historical cleanup of §79: schema alignment only. **Before:** tree clean at `ee277d3`; repository head `0023_second_local_provider`; live store `0022_runtime_provenance`. **Backup:** credential pre-flight passed; on-demand encrypted incremental `20260913-200405F_20260918-222510I` (22:25:10 to 22:26:17 CDT, `completed successfully`, stanza `ok`, cipher aes-256-cbc, off-machine repository), chained on the full of 13 September; kept. **Scope of 0023, read before applying:** drops and recreates the one check constraint `ck_model_calls_known_cost_is_recorded`, widening `provider = 'lmstudio'` to `provider IN ('lmstudio', 'llamacpp')`; no UPDATE, no DELETE, no data statement, no table, column, routing, admission or profile change; downgrade restores the earlier clause and refuses while a tokenless known-$0 `llamacpp` row exists, as CI exercises. **Applied:** `uv run alembic -x deploy=live upgrade head`, 23:13 CDT, `Running upgrade 0022_runtime_provenance -> 0023_second_local_provider`, exit 0. **Fingerprint, read-only transaction, before and after:** 37 lines, 35 identical, the two that differ being the revision and that constraint's definition. Identical: 23 tables; every table's row count (messages 152, conversations 36, model_calls 179, personas 7, projects 8, blind_positions 8, deliberations 8, classifications 34, classification_labels 20, classification_reviews 2, execution_events 21, budget_reservations 173, model_call_cache_usage 41, model_call_measurements 13, the rest 0); md5 over messages `29bc5d5126151578859b7fac83db6ee1`, conversation origins `15bbab4e3f5e110d73f062a784fffcf2`, calls `68f542ed44f414395e8a76d816b34e14`, call cost fields `06ca3523a8ea8606795a9b2c2a7c84d5`, personas `2fea30f7e67da831444580e8f5d19a1a`, projects `86c26aec5ad12fa1584a56ec6052e690`, blind positions `86a0207354dde08ae52d7172ea4c9b58`, execution events `0e9cbb3c74e8369a7dcde37e05e9cacf`; every other `model_calls` constraint definition `e94ece038141802950d3e81b79eb506d`; 23 constraints and 4 triggers on `model_calls`; active persona v1.8 revision 7. **After:** chain 23 revisions, one root, one head, unbroken; focused tests on the scratch store green (schema, local hosting, persistence including the known-$0 clause, local startup, candidate lane: 216 passed; live-migration guard: 8 passed); service health `running`, no warnings. The service was not restarted: 0023 changes no code path the production service uses, and it has run since 16 September 19:06 on the code loaded then. No local-provider call exists on the live store and none was manufactured. Partner-profiled routes unchanged (`gpt-5-6-sol-medium`, `opus-5-medium`); no active local route; all four local entries `NOT_ADMITTED`, no profile, no fallback. No rollback was run.

## 81. Track C resumed — Attachment Substrate v1.2 implemented and image input built to the acceptance boundary — 19 September 2026

Owner ruling of 19 September 2026 (`04-layer-0.md` §5 amendment of that date). **Commits:** `3d04c3e` schema (migration `0024`, six append-only tables, every §5 rule structural — a blob's key is the digest of its bytes as a check constraint, composite keys keep one file's tree one file's tree, partial unique indexes hold one `started` and one terminal event per attempt); `2c3b2e8` admission, derived representations and **the provider-neutral ordered content-part boundary** (a turn is a sequence of parts, `content` derived from them so there is one source of truth, the union discriminated so audio and video are additive); `7731d1e` Sol verified image-capable and the reservation widened; `bf55863` the OpenAI adapter; `508c8ff` the gateway (atomic evidence commit, strictest-of classification, derive-once, binding, current-turn visual state); `7908041` the deliberated path and the same-ordered-image proof for blind and final; `2232a73` the API; `5fbace4` the desktop. **Provider verification** (`docs/reviews/evidence/2026-09-19-sol-image-input.json`): four probes, $0.018072 of the authorised $0.50; against a measured text-only baseline of 18 tokens the documented formula is exact within the patch budget (512x512 predicted 308 measured 308; 1024x768 predicted 922 measured 922) and one token above the cap when the provider resizes (2048x2048 predicted 3000 measured 3001) — resolved by never transmitting an image the provider must resize, at a 1,600 long edge where the arithmetic is exact. **Tests:** 1,127 Python and 54 desktop, full mirror green; the schema transcription, both record-state key-set pins and the frozen expectations extended additively, none rewritten. **Not done and not permitted here:** a multimodal classifier, audio, video, documents, a local image route, any model download. **WP-0.11 remains open** and still blocks operational visual use and visual gate evidence. The live migration of `0024` and the bounded acceptance sequence await the owner's authorisation.

## 82. Track C pre-live corrections — the transmission rule, processing-event honesty, provider versus house limits — 20 September 2026

Owner return of 20 September 2026, before migration `0024` touched the live store. **First-party documentation re-read that day** (developers.openai.com images-vision): `high` "fits within 2048 x 2048 pixels and 2,500 patches"; patches are "32px x 32px"; the gpt-5.6 multiplier is 1.2; **no per-image size limit is documented at all**, only "Up to 512 MB total payload per request"; and the provider states that "floating-point rounding in billing can make the final count differ from the estimate by one token" — which is exactly the +1 the 2048x2048 probe measured. **The 1,600-pixel long-edge rule is withdrawn**: right for a square, wrong as the aspect ratio departs from it, and it reduced a 16:9 frame at 2048x1152 (2,304 patches, already inside the budget) to 1600x900 for no accounting reason. Replaced by `fit_within_limits`, which takes the largest aspect-preserving size inside both documented limits — long edge as the search variable, short edge floored, largest feasible long edge, never upscaling, deterministic and reproducible. It does not reproduce the provider's shrink. The one-token rounding is a declared **reservation margin** in `val_policy.budget`, never described as price. **Processing events now describe actual processing**: an unchanged original records no `derive:model_input_image` attempt at all, a real derivation records `started` then `succeeded` naming its representation, a real failure records `started` then `failed` with its reason, and `verify` keeps its distinct meaning; migration `0024` was strengthened in place so a succeeded derivation structurally requires a representation of that attachment. **Provider facts and house policy are separate objects** on the capability declaration, so the house's 20 MB per-image ceiling is recorded as house admission policy with its reason rather than attributed to a provider that documents none. **The paid probe evidence is corrected, not rewritten**: a note names the baseline-derived `measured_image_tokens` as authoritative, explains that the superseded `implied_image_tokens` used the conservative pre-call text bound the ruling required to be stated in advance, and every superseded field is preserved. **Tests that changed, and why:** the capability test's `edge == 1600` and `patches_per_edge ** 2 == 2500` encoded a transmission choice rather than a provider fact and now assert the provider's 2,048 bound; two substrate tests inserted a succeeded derivation producing nothing, which the strengthened constraint correctly refuses; one adapter test reached through the flattened capability shape. Full mirror green. **Migration `0024` remains unapplied to the live store.**

## 83. Track C final pre-live correction — request-level provider limits and the aggregate preflight — 20 September 2026

Owner return of 20 September 2026, the last bounded correction before live. **First-party re-read that day** (developers.openai.com images-vision): "Up to 1,500 images per request" and "Up to 512 MB total payload per request", still no per-image size limit, and — decisively — **the page nowhere defines what is measured against the payload ceiling**. The two limits are recorded as request-wide provider facts in their own object, with `payload_unit_is_documented=False` carrying the documentation gap as a fact about the documentation. The house counts the complete data URIs it would transmit, recorded separately with a reason beginning "HOUSE INTERPRETATION OF AN AMBIGUOUS PROVIDER FACT": raw bytes < base64 < data URI, so counting the largest image-attributable quantity it actually puts on the wire cannot make the house more permissive than a ceiling whose meaning is unsettled. `512 MB` is read as 512 x 10^6 rather than 512 MiB, the smaller reading being the conservative one. A test proves the computed measure equals the adapter's own construction byte for byte, so the guard measures the wire rather than a fiction. **Core verifies the aggregate** after every attachment is admitted and its transmitted representation derived and measured, and before any pixels leave the machine: it refuses naming the applicable limit, the measured state and which of the two was violated, drops no attachment, splits no turn, writes no binding claiming a transmission, and never reaches the adapter. **House per-image ceiling raised to 50 MB on the admitted original** by owner ruling, so character sheets, storyboards, frames and high-resolution PNGs are not refused; the decoded-pixel safety bound is separate and unchanged. **The reservation composes** over every transmitted image, each priced on its own dimensions with its own one-token margin, proven at one, two, three, five and twelve images and over a real multi-image turn. Ordering, per-act binding and per-digest binding are proven over three distinct images. Tests changed: the house-limit test for 50 MB and the original-file wording; the policy fixtures for the new objects. **No paid provider traffic was used.** Full mirror green. **Migration `0024` remains unapplied to the authoritative live store.**

---

## 84. Migration 0024 applied to the live store; the first genuine image turn; two live defects — 20 September 2026

**The migration.** `pgBackRest` backup taken first and verified. The first attempt **stopped on its own stop condition**: the backup timed out waiting sixty seconds for a WAL segment. The cause was mine — scratch-database churn on the same cluster, and WAL is cluster-wide — so the backlog was allowed to drain and the backup retried and succeeded. It was reported rather than worked around. `0024_attachment_substrate` then applied to the live store, fingerprint identical to the scratch application, and the desktop was rebuilt and installed.

**The first genuine image turn** (Lord Armand, conversation `01a0c109-88f9-7159-ac99-74fbc92830f4`). Original PNG 2752×1536, 8,371,957 bytes, digest `7fc13a7c5072b69a…`; derived by Pillow 12.3.0 to 2048×1143, 2,837,575 bytes, digest `278f4d6ee17a12e4…`; processing events exactly `started → succeeded`, the succeeded event naming the representation; binding `input_kind=representation` with `{"detail": "high"}`; the Sol call `complete`/`ok`, 8,221 in and 1,062 out, **$0.062342**, plus a Haiku classification at $0.000905 — **$0.063247 for the turn**. Val received the image and described it in detail. The substrate behaved exactly as contracted.

**Two defects he reported, both in the desktop and neither in the substrate.** The image never appeared in the thread: the webview's content security policy allowed `connect-src` to the loopback service so every fetch worked, but `img-src` was `'self' data:`, so an `<img>` at the governed byte route was refused before any request was made. The existing test asserted the URL *string* and passed while he saw nothing. And Val's answer displayed its own control syntax — `### Characters and action` shown literally — because the renderer put the message into a text node. Fixed by correcting `img-src` to permit exactly that origin and nothing wider, and by adding a deliberately small Markdown reader that yields React elements and never HTML, with no links and no images, over an untouched stored message.


---

## 85. Persona v1.9 revision 8 — form follows the task; two test corrections; a CI wiring defect found by pushing — 20 September 2026

**The ruling.** The first image answer came back as a report — a heading, then stacked sections — for an ordinary conversational question. The diagnosis was neither the model nor the route: **nothing governed default form.** §5 governed register, and "Explicit form governs the defaults" governed the case where he states a format; the ordinary case, where he states nothing, was unwritten, and the model filled it with its own habits. Lord Armand ruled one rule in his own approved wording, placed in §5 beside the register rule: form follows the task, and ordinary conversation is prose. Structure keeps its place where the material is genuinely of that shape; it has to earn it. A test guards the opposite over-correction, because a persona that refused a list when the material is a list would be as wrong as the report that prompted the ruling.

**Verification, deterministic only.** Reversing exactly the two ruled changes — the inserted rule and the version label — reproduces v1.8 byte for byte, digest `1608715f…`. The v1.7 reversal now chains through it, so one broken link names the version whose text moved. The `semantic_version` pins moved from 1.8 to 1.9 in five places in `test_persona.py` and two in `test_persona_capability_honesty.py`, and one test whose name asserted v1.8 was active was renamed to name the current persona instead; this was reported rather than changed silently. No provider call, no live persona turn, no qualification suite.

**Activated** 20 September 2026, 22:35 CDT: revision 8 (`01a0c20a-0774-74db-a4eb-e7533cf9bc69`, digest `33831189b493…`, 23,550 characters), after CI run 35557830345 passed on `46683bb`. Revisions 1–7 byte-preserved, each stored digest equal to the digest recomputed from its stored content; exactly one revision active; `verify_against_source` clean; the service restarted and reported running with no warnings.

**Two test corrections he asked for first.** The Markdown reader no longer reads underscores as emphasis: Val discusses column names and model identifiers constantly, and `model_call_image_inputs` was rendering with its middle in italics. This is the simpler of the two options he offered, taken because it satisfies the requirement completely; the subset is not otherwise broadened. And the attachment regression test now renders the production component rather than its own copy of the markup — the copy could have gone on passing while the shipped renderer diverged, which is the same failure as asserting a URL string. `Attachments` moved to its own module, imported by both the thread and the test; the policy half still parses the shipped Tauri configuration.

**A CI wiring defect, found by pushing.** This was the first push since 15 September, and the scope-ruling check failed on `ModuleNotFoundError: val_domain`: the 16 September amendment gave that check a registry lookup while its CI step ran under `uv run --no-project`. **My first fix was wrong and my verification did not verify.** I wrote that the registry imports nothing outside the standard library and put its source on the path; it imports pydantic, and CI failed again one import deeper. I had run the CI invocation locally, seen it pass, and reported it proven — but a developer shell resolves that invocation against an environment where the dependency is already present, which is the same reason the original omission survived four days. The honest fix is the one the boundaries job already uses: sync that step, because the ruling makes the registry the authority for what is routable and consulting it costs the dependency tree. CI green on all six jobs.

**State after:** live store at `0024_attachment_substrate`; persona revision 8 active; `gpt-5-6-sol-medium` the only image-capable routable configuration, six routable in total; desktop binary `00a06bd093dba259` installed with `Val (built 2026-09-20 pre-form-rule).app` preserved. **No paid provider call was made in this round.** The second acceptance turn was not performed, as instructed.
